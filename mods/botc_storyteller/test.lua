...
-- Killing a real player applies the transparency texture_mod
...
assert_eq(mock_players["Bob"].texture_mod, botc.DEAD_TEXTURE_MOD, "dead real player becomes ~0.4 alpha transparent")

-- Reviving a real player clears the transparency
...
assert_eq(mock_players["Bob"].texture_mod, "", "revived real player becomes fully opaque again")

-- A player who is dead when they (re)join gets the transparency reapplied
botc.ST.roles["Bob"].alive = false
mock_players["Bob"].texture_mod = nil -- simulate a fresh ObjectRef with no texture_mod set yet
simulate_join("Bob")
assert_eq(mock_players["Bob"].texture_mod, botc.DEAD_TEXTURE_MOD, "rejoining while dead reapplies the transparency texture_mod")

-- A player who is alive when they (re)join stays fully opaque
botc.ST.roles["Bob"].alive = true
mock_players["Bob"].texture_mod = nil
simulate_join("Bob")
assert_eq(mock_players["Bob"].texture_mod, "", "rejoining while alive stays fully opaque")
